...
Adoro.FormValidator2 = function(formNode, options) {
    var $formNode = $(formNode);// interesting
    // lexical scope
    var me = this;
...
(function(){
/*
var fn = document.getElementById("formNode1");
var fv = new FormValidator(fn, {
	oneErrorPerValidator: false//?
});

fv.addValidator("username", [
	// by adding a new instance of Rule
	new Rule(function(){
		// check min length
		},
		"The min length for username is wrong", 
		{length: 1}
	),
	// by object literal
	{
...
fv.addEventHandler("onValidateStart", function() {
	// do something general to all validators when they start	
});

var validators = fv.getValidators();

for(var i = validators.length; i >=0; i--) {
	fv.addEventHandler(validators[i].fieldName+".onValidateStart", somethingGenericForOnValidateStart)
}

function somethingGenericForOnValidateStart() {
	// show red cross next to field
}

fv.addEventHandler("username.onValidateStart", function() {
	// show error message next to username
	// dont show red cross next to username
});

fv.removeEventHandler("username.onValidateStart", somethingGenericForOnValidateStart);

var lastButtonClicked = null;

form.onsubmit = function() {
	if (lastButtonClicked === "whatever") {
		var valid = fv.validate();
		if(!valid) {
			// do some stuff;
			return false;
		}
...
document.getElementById("contextualSubmit").onclick = function() {
	var valid = fv.validate("postcode", "houseNumber")
	if(!valid) {
