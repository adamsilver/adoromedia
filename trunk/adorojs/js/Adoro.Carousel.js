...
Adoro.Carousel = function(container, options) {
	...
	if(typeof options === "object") {
		config.onStart = typeof options.onStart === "function" ? options.onStart : config.onStart;
		config.onComplete = typeof options.onComplete === "function" ? options.onComplete : config.onComplete;
		...
		config.indicatorsContainerClass = typeof options.indicatorsContainerClass === "string" ? options.indicatorsContainerClass : config.indicatorsContainerClass;
	...
	function onStart() {
		...
		config.onStart.call(ul);		
	}		
	
	function onComplete() {
		if(!config.onComplete) return;
		config.onComplete.call(ul);		
	}	
	...
	function getSlides(from, to) {
		...
		for(var i = 0; i < slides.length; i++) {
			if(	(typeof from === "number" && i < from) ||(typeof to === "number" && i>to-1) ) continue;
			returnSlides.push(slides[i]);
		}
		return returnSlides;
	}
